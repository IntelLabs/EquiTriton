...
class SecondOrderSphericalHarmonic(torch.autograd.Function):
    @staticmethod
    def forward(ctx, coords: torch.Tensor, mask: torch.Tensor | None = None, block_size: int = 64):
        ...
        # apply the kernel
        second_order_fwd[num_blocks,](coords, output_tensor, block_size, coord_numel, output_numel)
        ctx.save_for_backward(coords)
        return output_tensor
    
    @staticmethod
    def backward(ctx, sph_grad_tensor: torch.Tensor, block_size: int = 64) -> torch.Tensor:
        (coords,) = ctx.saved_tensors
        coord_grad_output = torch.zeros_like(coords)
        num_blocks = calculate_lastdim_num_blocks(coords, block_size)
        # call backward kernel
        second_order_bwd[num_blocks,](coords, coord_grad_output, sph_grad_tensor, block_size, coords.numel(), sph_grad_tensor.numel())
...
def second_order_fwd(
    ...
    tl.store(output_ptr + output_row_offset + 1, Y21, mask=output_row_offset + 1 < output_numel)
    tl.store(output_ptr + output_row_offset + 2, Y22, mask=output_row_offset + 2 < output_numel)
    tl.store(output_ptr + output_row_offset + 3, Y23, mask=output_row_offset + 3 < output_numel)
    tl.store(output_ptr + output_row_offset + 4, Y24, mask=output_row_offset + 4 < output_numel)
...
def second_order_bwd(
    ...
    g_Y20 = tl.load(sph_grad_ptr + output_row_offset, mask=output_row_offset < output_numel)
    g_Y21 = tl.load(sph_grad_ptr + output_row_offset + 1, mask=output_row_offset + 1 < output_numel)
    g_Y22 = tl.load(sph_grad_ptr + output_row_offset + 2, mask=output_row_offset + 2 < output_numel)
    g_Y23 = tl.load(sph_grad_ptr + output_row_offset + 3, mask=output_row_offset + 3 < output_numel)
    g_Y24 = tl.load(sph_grad_ptr + output_row_offset + 4, mask=output_row_offset + 4 < output_numel)
    g_x = CONST_00 * g_Y20 * z + CONST_00 * g_Y21 * y - CONST_01 * g_Y22 * x - CONST_00 * g_Y24 * x
    g_y = CONST_00 * g_Y21 * x + CONST_02 * g_Y22 * y + CONST_00 * g_Y23 * z
    g_z = CONST_00 * g_Y20 * x - CONST_01 * g_Y22 * z + CONST_00 * g_Y23 * y + CONST_00 * g_Y24 * z
    # write out gradients
